...
genearray.each do |geneid|
  ...
  emblfile.each_entry do |entry|
    chr_number, chr_ini, chr_fin = get_chr_info(entry.accession) unless entry.accession.nil?
    motifs = Hash.new()
    ...
    entry.features.each do |feature|
      if feature.feature == "exon"
        ...
        motif_loc_array.concat(motif_search("cttctt", exon))
        motif_loc_array.concat(motif_search("aagaag", exon, strand_change=true))
        motif_loc_array.each do |d_key|
          motifs[d_key.clone] = Array.new unless motifs.keys.include?(d_key)
          motifs[d_key.clone].append(exon[:exon_id])
        end
      end
    end
    next if motifs.empty?
    chrgff_output.puts "#{chr_number}\t.\tgene\t#{chr_ini}\t#{chr_fin}\t.\t+\t.\tID=#{geneid}"
    gene_with_motif.append(geneid)
    id = 0
    motifs.keys.each do |loc,strand|
      id +=1
      ...
      feature.append(Bio::Feature::Qualifier.new('notes', motifs[[loc,strand]].join(",")))
      feature.append(Bio::Feature::Qualifier.new('id', motifs[[loc,strand]][0]))
      entry.features << feature
    end
    
    entry.features.each do |feature|
      
      if feature.feature == "nucleotide_motif"

        genesgff_output.puts print_gff_line(feature, geneid, geneid)
        chrgff_output.puts print_gff_line(feature, chr_number, geneid, chr_ini=chr_ini)
      end
    end
  end
end
...
genes_wo_motif = genearray - gene_with_motif.uniq
